...
export function setting(key: string, fromGamification = false, isBool = false): any {
  const val = app.data[`${fromGamification ? 'fof-gamification' : 'gamification-extras'}.${key}`];
